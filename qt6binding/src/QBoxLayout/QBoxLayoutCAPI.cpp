...
extern "C" {
    ...
    void QBoxLayout_insertWidget(void* layout, int index, void* widget, int stretch, int alignment) {
        static_cast<BindQBoxLayout*>(layout)->insertWidget(
    ...
    void QBoxLayout_addSpacing(void* layout, int size) {
        static_cast<BindQBoxLayout*>(layout)->addSpacing(size);
    }

    void QBoxLayout_addStretch(void* layout, int stretch) {
        static_cast<BindQBoxLayout*>(layout)->addStretch(stretch);
    }

    void QBoxLayout_addStrut(void* layout, int size) {
        static_cast<BindQBoxLayout*>(layout)->addStrut(size);
    }

    void QBoxLayout_insertSpacing(void* layout, int index, int size) {
        static_cast<BindQBoxLayout*>(layout)->insertSpacing(index, size);
    }

    void QBoxLayout_insertStretch(void* layout, int index, int stretch) {
        static_cast<BindQBoxLayout*>(layout)->insertStretch(index, stretch);
    }

    // Direction management
    int QBoxLayout_direction(void* layout) {
        return static_cast<int>(static_cast<BindQBoxLayout*>(layout)->direction());
    }

    void QBoxLayout_setDirection(void* layout, int direction) {
        static_cast<BindQBoxLayout*>(layout)->setDirection(
            static_cast<QBoxLayout::Direction>(direction));
    }

    // Stretch management
    void QBoxLayout_setStretch(void* layout, int index, int stretch) {
        static_cast<BindQBoxLayout*>(layout)->setStretch(index, stretch);
    }

    bool QBoxLayout_setStretchFactorWidget(void* layout, void* widget, int stretch) {
        return static_cast<BindQBoxLayout*>(layout)->setStretchFactor(
            static_cast<QWidget*>(widget), stretch);
    }
    ...
    int QBoxLayout_stretch(void* layout, int index) {
        return static_cast<BindQBoxLayout*>(layout)->stretch(index);
    }

    // Layout properties
    int QBoxLayout_count(void* layout) {
        return static_cast<BindQBoxLayout*>(layout)->count();
    }

    void QBoxLayout_setSpacing(void* layout, int spacing) {
        static_cast<BindQBoxLayout*>(layout)->setSpacing(spacing);
    }

    int QBoxLayout_spacing(void* layout) {
        return static_cast<BindQBoxLayout*>(layout)->spacing();
    }

    // Callbacks
    void QBoxLayout_setGeometryChangedCallback(void* layout, void (*callback)()) {
        auto* bindLayout = static_cast<BindQBoxLayout*>(layout);
        QBoxLayoutHandler* handler = bindLayout->handler();
        if (!handler) {
            handler = new QBoxLayoutHandler(bindLayout);
            bindLayout->setHandler(handler);
        }
